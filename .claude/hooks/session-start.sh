...
GODOT_URL="https://github.com/godotengine/godot/releases/download/${GODOT_VERSION}/${GODOT_NAME}.zip"
...
if [ ! -x "${GODOT_BIN}" ]; then
  ...
  for delay in 2 4 8 16 0; do
    if curl -fsSL --retry 2 -o "${tmp_zip}" "${GODOT_URL}"; then break; fi
    [ "${delay}" -eq 0 ] && { echo "Godot download failed" >&2; exit 1; }
    sleep "${delay}"
  done
